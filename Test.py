...
def run_check_live_posts():
    ...
    for item in tree.get_children():
        ...
        if post_id != "N/A" and post_id != "Không áp dụng":
...
def handle_check_button():
    """Kiểm tra nếu Treeview trống thì mở file, nếu không thì kiểm tra live post."""
    if not tree.get_children():  # Kiểm tra xem Treeview có dữ liệu không
        load_file()  # Gọi hàm chọn file nếu chưa có dữ liệu
    else:
        check_live_posts() 
def load_file():
...
check_live_button = Button(button_frame, text="Kiểm Tra Live Post", font=("Arial", 12), bg="Orange", fg="white", width=20, command=handle_check_button)
